...
case $duration_choice in
    1) duration_seconds=$((1 * 120)) ;;  # 1 hour
...
sudo tshark -i en0 -a duration:$duration_seconds -w /Users/morgansinan/Desktop/XML/captured_traffic.pcapng

(tshark -r captured_traffic.pcapng -T fields -e frame.number | wc -l) > /Users/morgansinan/Desktop/XML/total.txt

# Step 2: Extract DNS queries
tshark -r captured_traffic.pcapng -Y "dns.qry.name" -T fields -e dns.qry.name > /Users/morgansinan/Desktop/XML/dns.txt

# Step 3: Extract HTTP requests
tshark -r captured_traffic.pcapng -Y "http.request.full_uri" -T fields -e http.request.full_uri > /Users/morgansinan/Desktop/XML/http.txt
...
echo "----------------------------------"
tshark -r captured_traffic.pcapng -T fields -e ip.src -e ip.dst -e frame.len \
...
echo "---------------------------------------------"
tshark -r captured_traffic.pcapng -q -z conv,ip \
...
echo "------------------------------------"
tshark -r captured_traffic.pcapng -Y "tcp.flags.reset==1" -T fields -e ip.src \
...
echo "------------------------"
tshark -r captured_traffic.pcapng -Y 'data' -T fields -e ip.src -e ip.dst -e data.data \
    | head -20 \
...
echo "---------------------"
tshark -r captured_traffic.pcapng -Y "arp" -T fields -e arp.src.hw_mac -e arp.src.proto_ipv4 \
    | sort | uniq -c | sort -nr \
    | head -10 \
